...
class WayPointInfoWindow(layoutResId: Int, private val mTitleId: Int, private val mDescriptionId: Int, private val mSubDescriptionId: Int,
                         private val mSubDescription: String?, mapView: MapView) : BasicInfoWindow(layoutResId, mapView) {

    @SuppressLint("ClickableViewAccessibility")
    override fun onOpen(item: Any) {
        val overlayItem = item as OverlayItem
        var title: String? = overlayItem.title
        if (title == null)
            title = ""
        if (mView == null) {
        ...
        var snippet: String? = overlayItem.snippet
        if (snippet == null)
            snippet = ""
        val snippetHtml = Util.fromHtml(snippet.replace("href=\"//", "href=\"http://"))
        ...
        if (mSubDescription != null && "" != mSubDescription) {
            subDescText.text = Util.fromHtml(mSubDescription)
